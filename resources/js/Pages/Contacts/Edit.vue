    ...
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        ...
        <div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
          <button v-if="!contact.deleted_at" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy(false)">Delete Contact</button>
          <button v-if="contact.deleted_at" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy(true)">Delete Forever</button>
...
export default {
  ...
  methods: {
    ...
    destroy(force = false) {
      if (confirm(`Are you sure you want to delete this contact${force ? ' forever' : ''}?`)) {
        this.$inertia.post(`/contacts/${this.contact.id}/delete${force ? '?force=true' : ''}`)
      }
    },
    restore() {
      if (confirm('Are you sure you want to restore this contact?')) {
        this.$inertia.post(`/contacts/${this.contact.id}/restore`)
      }
    },
  },
}
