    ...
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        ...
        <div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
          <button v-if="!organization.deleted_at" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy(false)">Delete Organization</button>
          <button v-if="organization.deleted_at" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy(true)">Delete Forever</button>
...
export default {
  ...
  methods: {
    update() {
      this.form.post(`/organizations/${this.organization.id}`)
    },
    destroy(force = false) {
      if (confirm(`Are you sure you want to delete this organization${force ? ' forever' : ''}?`)) {
        this.$inertia.post(`/organizations/${this.organization.id}/delete${force ? '?force=true' : ''}`)
      }
    },
    restore() {
      if (confirm('Are you sure you want to restore this organization?')) {
        this.$inertia.post(`/organizations/${this.organization.id}/restore`)
      }
    },
  },
}
